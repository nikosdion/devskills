---
name: japitoken
description: Retrieve a valid Joomla API token from a local development Joomla site for testing API endpoints. Only works on local development sites (localhost, *.{{JOOMLA_SITE_DOMAIN}}, *.local.web, or sites under {{JOOMLA_SITES_DIR}}).
---
...
- Domain is `{{JOOMLA_SITE_DOMAIN}}` or a subdomain (e.g., `*.{{JOOMLA_SITE_DOMAIN}}`)
- Domain is `local.web` or a subdomain (e.g., `*.local.web`)
- Site root is under `{{JOOMLA_SITES_DIR}}`
...
3. If no virtual host config is found and site is under `{{JOOMLA_SITES_DIR}}`, extract domain from directory basename and assume `<basename>.{{JOOMLA_SITE_DOMAIN}}`
