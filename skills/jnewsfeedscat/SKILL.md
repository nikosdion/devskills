---
name: jnewsfeedscat
description: Manage Joomla newsfeed categories via the Joomla Web Services API. Supports listing, filtering, getting, creating, updating, and deleting newsfeed categories. Only works on local development sites.
argument-hint: [site_path] [action] [options]
---

# Joomla Newsfeed Categories Manager

Manage Joomla newsfeed categories through the Joomla Web Services API on a local development site.
...
**Request:**
```
GET <baseUrl>/api/index.php/v1/newsfeeds/categories[?filter[published]=1&...]
Authorization: Bearer <token>
Accept: application/vnd.api+json
```
...
### get — Get a single newsfeed category
...
**Request:**
```
GET <baseUrl>/api/index.php/v1/newsfeeds/categories/<id>
Authorization: Bearer <token>
Accept: application/vnd.api+json
```
...
### create — Create a new newsfeed category
...
**Request:**
```
POST <baseUrl>/api/index.php/v1/newsfeeds/categories
Authorization: Bearer <token>
Accept: application/vnd.api+json
...
### update — Update an existing newsfeed category
...
**Request:**
```
PATCH <baseUrl>/api/index.php/v1/newsfeeds/categories/<id>
Authorization: Bearer <token>
Accept: application/vnd.api+json
Content-Type: application/json
...
### delete — Delete a newsfeed category
...
**Delete request:**
```
DELETE <baseUrl>/api/index.php/v1/newsfeeds/categories/<id>
Authorization: Bearer <token>
Accept: application/vnd.api+json
```
...
- Newsfeed categories use the field name `published` (not `state`) for publication status.
- Use `jnewsfeeds` to manage the newsfeeds within these categories.
