---
name: installjoomla
description: Download and install Joomla CMS 5+ into a local directory, selecting the closest matching stable release from Joomla GitHub releases, provisioning local MySQL defaults when missing, running CLI installation, and verifying frontend access on the corresponding *.{{JOOMLA_SITE_DOMAIN}} hostname.
...
- If `target_dir` is a bare name (no `/`), install to `{{JOOMLA_SITES_DIR}}/<name>`.
- If `site_name` is missing, use the directory basename.
- If admin fields are missing, use:
- `admin_name`: `{{JOOMLA_ADMIN_NAME}}`
- `admin_username`: `{{JOOMLA_ADMIN_USERNAME}}`
- `admin_password`: `{{JOOMLA_ADMIN_PASSWORD}}`
- `admin_email`: `{{JOOMLA_ADMIN_EMAIL}}`
...
1. Compute hostname from directory basename: `https://<basename>.{{JOOMLA_SITE_DOMAIN}}`
